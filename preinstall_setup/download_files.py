...
lang_pkgbuild_dir = os.path.join(
    os.path.dirname(os.path.dirname(filepath)), "pkgbuilds", "Languages"
)
deps_pkgbuild_dir = os.path.join(
    os.path.dirname(os.path.dirname(filepath)), "pkgbuilds", "Dependencies"
)
...
class DownloadTarget(object):
    def __init__(self, kind, name, hash, url, filename=None):
        self.kind = kind  # One of "lang" or "dep"
        self.name = name
        if self.kind == "lang":
            self.dirname = os.path.join(lang_pkgbuild_dir, name)
        elif self.kind == "dep":
            self.dirname = os.path.join(deps_pkgbuild_dir, name)
        else:
